...
0. Clarify the user's intent if it is not explicit:
   - If the user provides data but does NOT specify what to do with it (e.g. "帮我处理一下", "看看这个数据"), do NOT guess. Ask: "What would you like me to do with this data? Options: convert JSON to CSV, flatten nested fields, filter/extract specific fields, aggregate/group, or something else?"
   - If the user's request is vague (e.g. "process this", "look at this data"), list the available operations before proceeding.
